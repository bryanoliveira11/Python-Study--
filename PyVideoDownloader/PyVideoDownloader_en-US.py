...
class PythonDownloader(QMainWindow,Ui_PytubeDownloader):
    def __init__(self,parent = None):
        ...
        self.Video_URL = ''
        self.Video_Title = ''
        self.url_info.setText("Copy and Paste the Youtube Video or Playlist Link Here :")
    ...
    def SearchVideoURL(self): # search video and validate re
        ...
        try: 
            ...
            self.Url_Exception.setText(f'[ERROR] : Not Possible to Search the URL. Try Again ! <br/>Make Sure that the Video/Playlist is not Private.')
...
class OptionsWindow(Ui_DownloaderOptions,QMainWindow):
    ...
    def SelectDownload_Path(self): # functions to select a folder for downloads and create a txt to save it
        ...
        with open('DownloadPath.txt','w') as DownloadPath: # will create a txt file and save the folder inside
            if not self.Download_Folder:
                self.path_show.setText('[INFO] : Your Download Path is Empty ! ')
                return
    ...
    def getDownloadPathFromTxt(self): # that function will open the txt DownloadPath and get the path thats on it to show at screen.
        try:
            ...
            self.Download_Exception.setText('[INFO] : Choose a Path to Download Before Start Downloading !')
    ...
    def StartDownloadValidations(self):
        try:
            ...
            self.Download_Exception.setText('[INFO] : Choose a Path to Download First !')
            return
        
        if self.MainWindowAttributes.Type_of_URL == 'PLAYLIST' and self.Playlist_Download == None or self.Resolution_Download == None:
            self.Download_Exception.setText('[INFO] : Check all the Options to Download !')
            return
    ...
    def ValidateInputs_Interval(self):
        try:
            ...
            if self.StartInterval > self.EndInterval or self.StartInterval > self.MainWindowAttributes.Quantity_of_Videos or self.EndInterval > self.MainWindowAttributes.Quantity_of_Videos or self.StartInterval < 0 or self.EndInterval < 0 or self.StartInterval == 0 or self.EndInterval == 0:
                self.Download_Exception.setText(f"[INFO] : Input Only Bigger Than 0 and in Range {self.MainWindowAttributes.Quantity_of_Videos} Numbers ! Start Must be < End ")
                self.Start_Num.setText('')
                self.End_Num.setText('')
            elif self.StartInterval == self.EndInterval:
                self.Download_Exception.setText(f"[INFO] : Your Start Can't be the Same as Your End ")
                self.End_Num.setText('')
            else:
                self.DownloadIntervalPlaylist()

        except ValueError:
            self.Download_Exception.setText(f"[INFO] : Input Only Numbers ! ")
            self.Start_Num.setText('')
            self.End_Num.setText('')
...
class DownloadSingleVideo_Thread(QThread): # class for downloading a single video thread
    ...
    def run(self):
        try:
            ...
            self.Status_Text.emit("#2c2c2c",f"[STATUS] : DOWNLOADING ... ")
            ...
            self.Status_Text.emit("#1aa839","[SUCESS] : VIDEO DOWNLOADED SUCCESFULLY ! ")

            time.sleep(1.2)

        except Exception:
            self.Update_Gif.emit(".\\designs\\bkp_ui\\../../imgs/error_gif.gif")
            self.Status_Text.emit("#7a1c15","[ERROR] : NOT POSSIBLE TO DOWNLOAD  !  THE VIDEO PROBABLY IS NOT PUBLIC OR HAS AGE RESTRICTIONS OR IS UNAVAILABLE / BLOCKED BY REGION.")
    ...
    def run(self):
        ...
        for videos in self.youtube_video.videos: # optimal download when stream is none
            try:
                ...
                self.Status_Text.emit("#2c2c2c",f"[STATUS] : DOWNLOADING VIDEO {self.VideoCount} of {self.QuantityVideos} From PLAYLIST ... ")
                ...
                self.VideoCount += 1
                videos.register_on_complete_callback(self.Status_Text.emit("#1aa839","[SUCESS] : VIDEOS DOWNLOADED SUCCESFULLY ! "))
        ...
        if self.SkippedVideos != []: # shows videos that were skipped because of unavailability in youtube
            self.Status_Text.emit("#7a1c15",f"[INFO] : THE FOLLOWING VIDEO(S) WERE UNAVAILABLE TO DOWNLOAD : {self.SkippedVideos}")
    ...
    def run(self):
        ...
        for self.URL_Counter in range (len(self.List_Interval_URLS)): # set a while loop based on counter to get the url link from list counter as a string
            try:
                ...
                self.Update_Gif.emit(".\\designs\\bkp_ui\\../../imgs/loading_gif.gif")
                self.ShowVideoTitle.emit(f'PLAYLIST : {self.youtube_video.title} - ({self.QuantityVideos} VIDEOS)')
                self.Status_Text.emit(f"#2c2c2c",f"DOWNLOADING INTERVAL VIDEO {self.VideoCount} of {self.End} FROM PLAYLIST ...")
                ...
                self.VideoCount += 1
                self.Current_URL.register_on_complete_callback(self.Status_Text.emit("#1aa839",f"[SUCESS] : VIDEOS {self.Start} to {self.End} of PLAYLIST DOWNLOADED SUCCESFULLY ! "))
